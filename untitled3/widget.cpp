...
Widget::Widget(int argc, char **argv, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Widget),
    binfile(new char[100]),
    softName(new char[100])
{

    readArgs(argc,argv);
    ui->setupUi(this);
    this->setWindowTitle(QString(softName));
    mygetTop=new getTop(binfile,n,m);//-----------new
    mypscons=new scons[n];
    binfile=new char[100];
    readArgs(argc,argv);
...
void Widget::readArgs(int argc, char **argv)//filename,n,m,binfile,port
{
    if(argc!=6)
        this->close();
    else
...
void Widget::setLables(const QString &filename)
{
    ...
    mygetTop->getValue(c_filename,mypscons,port);//fill mypscons with top simulate pictures
    for(int i=0;i<10;i++)
